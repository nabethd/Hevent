...
class CalendarSyncManager(private val context: Context) {
     ...
     * Events are tagged using ExtendedProperties so targeted deletions only remove
     * events created by this application.
    ...
    suspend fun insertEvents(
        ...
        var insertedCount = 0
        // Process in batches of 20 to accommodate both Event and ExtendedProperty operations safely
        val chunkSize = 20
        ...
        for (chunk in chunks) {
            ...
            try {
                ...
                // Filter for events inserted (every batch contains event inserts and property inserts)
                insertedCount += results.count { it.uri != null && it.uri.toString().contains("events") }
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
